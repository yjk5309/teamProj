...
def IndividualQuestionView(request, store_id):
    ...
    if request.method == "GET":
        un_answered_question_counts = execute_and_get("SELECT COUNT(*) FROM individual_question WHERE store_id = (%s) AND answer_complete = (%s)",
                                (store_id, 0,))
        ...
        question_count = {
            'un_answered_question_counts': un_answered_question_counts[0][0],
            'answered_question_counts': answered_question_counts[0][0],
        }

        question_datas = execute_and_get("SELECT customer_id, title, content, answer, time, answer_complete FROM individual_question " +
        ...
        return redirect('seller:individual_question')